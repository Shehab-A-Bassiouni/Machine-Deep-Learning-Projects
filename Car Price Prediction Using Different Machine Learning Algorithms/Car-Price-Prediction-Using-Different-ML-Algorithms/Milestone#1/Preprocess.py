...
class Preprocessing:
    ...
    def standarizing(self):
        ...
        self.df[["mileage(kilometers)", "volume(cm3)", "year"]] = scaler.fit(
            self.df[["mileage(kilometers)", "volume(cm3)", "year"]]).transform(
            self.df[["mileage(kilometers)", "volume(cm3)", "year"]])
        if self.flag:
            self.df = pd.DataFrame(self.df,
                                   columns=['car_id', 'condition', 'mileage(kilometers)', 'fuel_type', 'volume(cm3)',
                                            'color', 'transmission', 'drive_unit', 'segment', 'price(USD)', 'model',
                                            'manufacturer', 'year'])
        else:
            self.df = pd.DataFrame(self.df,
                                   columns=['car_id', 'condition', 'mileage(kilometers)', 'fuel_type', 'volume(cm3)',
...
for i in range(0, len(train_columns)):
    if (not (train_columns[i] in test_columns)) and train_columns[i] != "price(USD)":
